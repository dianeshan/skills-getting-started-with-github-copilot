        ...
        const participantsMarkup =
          details.participants.length > 0
            ? `<ul class="participants-list">${details.participants
                .map(
                  (email) =>
                    `<li class="participant-pill"><span>${email}</span><button type="button" class="participant-delete" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Unregister ${email}" title="Unregister participant">&times;</button></li>`
                )
                .join("")}</ul>`
